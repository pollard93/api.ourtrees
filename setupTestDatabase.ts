...
export default async () => {
  ...
  process.env.DATABASE_URL_WRITE = process.env.DATABASE_URL_WRITE!.replace(
    /development|production/g,
    'test',
  );
  process.env.DATABASE_URL_READ = process.env.DATABASE_URL_READ!.replace(
    /development|production/g,
    'test',
  );
  ...
  const deleteDatabase = () =>
    new Promise((res, rej) => {
      ...
      con.connect((err) => {
        if (err) rej(err);
        con.query(
          `DROP DATABASE \`${global.process.env.DATABASE_URL_WRITE!.split('/').pop()}\``,
          (error, results) => {
            if (error) {
