    ...
    return context.db.write.treeData.update({
      ...
      data: {
        careDifficultyResult: {
          upsert: {
            create: {
              count: await context.db.read.treeDataCareDifficultyVote.count({
                where: { treeDataId },
              }),
              easy: await context.db.read.treeDataCareDifficultyVote.count({
                where: { treeDataId, type: TREE_CARE_DIFFICULTY_TYPE.EASY },
              }),
              moderate: await context.db.read.treeDataCareDifficultyVote.count({
                where: { treeDataId, type: TREE_CARE_DIFFICULTY_TYPE.MODERATE },
              }),
              hard: await context.db.read.treeDataCareDifficultyVote.count({
                where: { treeDataId, type: TREE_CARE_DIFFICULTY_TYPE.HARD },
              }),
            },
            update: {
              count: await context.db.read.treeDataCareDifficultyVote.count({
