        ...
        careWaterResult: {
          upsert: {
            create: {
              ...
              weekly: await context.db.read.treeDataCareWaterVote.count({ where: { treeDataId, type: TREE_CARE_WATER_TYPE.WEEKLY } }),
              biweekly: await context.db.read.treeDataCareWaterVote.count({ where: { treeDataId, type: TREE_CARE_WATER_TYPE.BIWEEKLY } }),
              triweekly: await context.db.read.treeDataCareWaterVote.count({ where: { treeDataId, type: TREE_CARE_WATER_TYPE.TRIWEEKLY } }),
            },
            update: {
              count: await context.db.read.treeDataCareWaterVote.count({ where: { treeDataId } }),
              weekly: await context.db.read.treeDataCareWaterVote.count({ where: { treeDataId, type: TREE_CARE_WATER_TYPE.WEEKLY } }),
