      ...
      const getUser = async () => {
        switch (provider) {
          case 'FACEBOOK':
            ...
            const facebookAccessToken = (context.req.headers.authorization || '').replace(
              'Bearer ',
              '',
            );
            ...
            const fbUser: { id: string; email: string } =
              process.env.NODE_ENV !== 'test'
                ? await (
                    await fetch(
                      `https://graph.facebook.com/me?fields=email,name&access_token=${facebookAccessToken}`,
                    )
                  ).json()
          ...
          case 'GOOGLE':
            ...
            const googleUser: { sub: string; email: string } =
              process.env.NODE_ENV !== 'test'
                ? await (
                    await fetch(`https://oauth2.googleapis.com/tokeninfo?id_token=${googleIdToken}`)
                  ).json()
