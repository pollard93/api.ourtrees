import {
  TreeDataCareGerminationNotesContent,
...
export class TreeDataCareGerminationNotesResultProfileResolver {
  ...
  async content(
    @Root() { id }: TreeDataCareGerminationNotesResult,
    @Ctx() context: Context<null>,
  ): Promise<TreeDataCareGerminationNotesContent | null | undefined> {
    return (
      await context.db.read.treeDataCareGerminationNotesResult.findUnique({
        where: { id },
