import {
  TreeDataCareObtainingSeedsContent,
...
export class TreeDataCareObtainingSeedsResultProfileResolver {
  ...
  async content(
    @Root() { id }: TreeDataCareObtainingSeedsResult,
    @Ctx() context: Context<null>,
  ): Promise<TreeDataCareObtainingSeedsContent | null | undefined> {
    return (
      await context.db.read.treeDataCareObtainingSeedsResult.findUnique({
        where: { id },
