 ...
 */
type NotificationDataType<T extends NOTIFICATION_TYPE> = T extends 'PASSWORD_CHANGED'
  ? { someData: string }
  : null;
 ...
 */
export type DBNotificationArgs<T extends NOTIFICATION_TYPE> = NotificationDataType<T> extends null
  ? {
      type: T;
      receiver: User;
      message: string;
 ...
 * If NotificationDataType returns null then NotificationArgs.data is not required
 */
export type NotificationArgs<T extends NOTIFICATION_TYPE> = NotificationDataType<T> extends null
  ? {
      type: T;
      receivers: User[];
      sender?: User;
 ...
 */
export type PushNotificationDataType<T extends NOTIFICATION_TYPE> = T extends 'PASSWORD_CHANGED'
  ? { someData: string } & PushNotificationData<T>
